...
func newWorker(config *Config, chainConfig ctypes.ChainConfigurator, engine consensus.Engine, eth Backend, mux *event.TypeMux, isLocalBlock func(header *types.Header) bool, init bool) *worker {
	...
	// Wire custom-coinbase work generation for olivetumhash remote miners.
	if setter, ok := engine.(interface {
		SetWorkForProducer(olivetumhash.WorkForProducer)
	}); ok {
		setter.SetWorkForProducer(worker.produceWorkFor)
	}
...
func (w *worker) isOlivetumhashEngine() bool {
	switch w.engine.(type) {
	case *olivetumhash.Olivetumhash:
		return true
	default:
		return false
	}
	...
	for {
		...
		case <-timer.C:
			...
			if w.isRunning() && (!w.chainConfig.GetConsensusEngineType().IsClique() || w.chainConfig.GetCliquePeriod() > 0) {
				// Short circuit if no new transaction arrives, except for Olivetumhash
				// where periodic resubmits are required to refresh timestamps/difficulty.
				if w.newTxs.Load() == 0 && !w.chainConfig.GetConsensusEngineType().IsOlivetumhash() && !w.isOlivetumhashEngine() {
					timer.Reset(recommit)
					continue
				}
				timestamp = time.Now().Unix()
...
func (w *worker) mainLoop() {
	...
	for {
		...
		case ev := <-w.chainSideCh:
			if w.chainConfig.GetConsensusEngineType().IsOlivetumhash() || w.isOlivetumhashEngine() {
				// Olivetumhash consensus does not permit uncles, so we ignore all
				// sidechain notifications instead of trying to collect them.
				continue
			}
...
func (w *worker) prepareWork(genParams *generateParams) (*environment, error) {
	...
	// Accumulate uncles only when the consensus permits it. Olivetumhash explicitly
	// forbids uncles, so ensure we never include them on Olivetum to avoid
	// producing blocks other peers will reject with "uncles not supported".
	allowUncles := !genParams.noUncle && !w.chainConfig.GetConsensusEngineType().IsOlivetumhash() && !w.isOlivetumhashEngine()
	if allowUncles {
...
// produceWorkFor builds a sealing block for a custom coinbase requested via
// olivetumhash_getWorkFor, registers it as a pending task, and returns the block
// plus the result channel that should receive the sealed block.
func (w *worker) produceWorkFor(address common.Address) (*types.Block, chan<- *types.Block, error) {
	params := &generateParams{
	...
	// Build the full block; custom requests are expected to be complete.
	if err := w.fillTransactions(nil, work); err != nil {
		work.discard()
		return nil, nil, err
	}
	env := work.copy()
	block, err := w.engine.FinalizeAndAssemble(w.chain, env.header, env.state, env.txs, env.unclelist(), env.receipts, params.withdrawals)
	work.discard()
	if err != nil {
		return nil, nil, err
	}
